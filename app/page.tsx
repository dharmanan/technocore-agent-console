...
export default function Home() {
  ...
  async function run(label: string, action: () => Promise<void>) {
    setBusy(label);
    try { await action(); }
    catch (error) { addEvent("Action failed", error instanceof Error ? error.message : "Unknown error", "warn"); }
  ...
  async function handlePublishProfile() {
    if (!identity) return;
    if (!mailbox) throw new Error("Create a mailbox first.");
  ...
  function handleImport(file: File) {
    ...
    reader.onload = () => {
      try {
        ...
        addEvent("Import failed", error instanceof Error ? error.message : "Invalid JSON file", "warn");
      }
    };
      ...
      <section className="grid">
        <article className="panel identityPanel">
          ...
          <div className="actions">
            {!identity ? <button className="primary" disabled={!!busy} onClick={handleCreateIdentity}>Generate DID</button> : <button onClick={() => exportIdentity(identity)}>Export private key</button>}
            <button onClick={() => importRef.current?.click()}>Import identity</button>
            {identity && <button className="danger" onClick={() => { clearIdentity(); setIdentity(null); addEvent("Local identity removed", "Export it first if you intend to reuse this DID.", "warn"); }}>Forget local key</button>}
